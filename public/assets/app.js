...
function updateThresholdForm(thresholds) {
  if (!thresholdForm || !thresholds) return;
...
function setControlsEnabled(enabled) {
  ...
  if (thresholdStatus) {
    thresholdStatus.textContent = enabled
      ? 'Seuils modifiables (session active).'
...
function updateAuthUI(user) {
  state.user = user;
  const name = user?.displayName || user?.username || 'Invite';
  if (authStatus) {
    authStatus.textContent = `Session: ${name}`;
  }
